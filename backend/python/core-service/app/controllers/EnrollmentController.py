...
async def _validate_completed_payment(request: Request, user_id: str, course_ids: list[str], payment_id: str) -> bool:
    if not ObjectId.is_valid(payment_id):
        return False

    payment = await payment_client.get_payment(payment_id, request)
    if not payment:
        return False
    if payment.get("user_id") != user_id or payment.get("status") != "completed":
        return False
...
async def _sync_completed_payment_enrollments(db, request: Request, user_id: str):
    ...
    payments = [
        payment
        for payment in await payment_client.list_my_payments(request)
        if payment.get("status") == "completed" and payment.get("course_ids")
    ]
    ...
    for payment in payments:
        ...
        await create_enrollments(
            ...
            user_id,
            course_ids,
            str(payment.get("_id") or payment.get("id")),
        )
...
async def enroll(
    ...
    if not await _validate_completed_payment(request, user["_id"], target_course_ids, payment_id):
...
async def my_courses(request: Request, db=Depends(get_db), user=Depends(get_current_user)):
    enrollments = (
    ...
    if not enrollments:
        await _sync_completed_payment_enrollments(db, request, user["_id"])
        enrollments = (
            await db["enrollments"]
            .find({"user_id": user["_id"], "payment_id": {"$exists": True}})
            .to_list(length=100)
