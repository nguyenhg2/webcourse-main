...
async def _build_profile_stats(db, user: dict, request: Request) -> dict:
    user_id = user["_id"]
    ...
    if role == "operator":
        payments = await payment_client.list_payments(request)
        return {
            "orders": len(payments),
            "completed_orders": len([payment for payment in payments if payment.get("status") == "completed"]),
            "pending_orders": len([payment for payment in payments if payment.get("status") == "pending"]),
        }
