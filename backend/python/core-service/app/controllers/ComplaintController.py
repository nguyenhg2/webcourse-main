...
async def _enrich_complaint(db, complaint: dict, request: Request) -> dict:
    ...
    if ObjectId.is_valid(str(item.get("payment_id", ""))):
        payment = await payment_client.get_payment(item["payment_id"], request)
...
async def create_complaint(
    ...
    return await _enrich_complaint(db, created, request)
...
async def update_complaint(
    ...
    return await _enrich_complaint(db, complaint, request)
