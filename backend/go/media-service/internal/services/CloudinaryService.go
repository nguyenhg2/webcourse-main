...
func EnsureCloudinaryConfig(cfg *config.Config) error {
	if cfg.CloudinaryCloud == "" || cfg.CloudinaryKey == "" || cfg.CloudinarySecret == "" {
		return models.APIError{Status: http.StatusInternalServerError, Message: "Thiếu cấu hình Cloudinary"}
	}
	return nil
}
...
func CloudinaryPostForm(endpoint string, form url.Values) ([]byte, error) {
	...
	if err != nil {
		return nil, models.APIError{Status: http.StatusBadGateway, Message: err.Error()}
	}
...
func readCloudinaryResponse(resp *http.Response) ([]byte, error) {
	...
	if resp.StatusCode >= http.StatusMultipleChoices {
		return nil, models.APIError{Status: resp.StatusCode, Message: string(body)}
	}
	return body, nil
}
